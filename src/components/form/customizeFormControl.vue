...
export default {
  ...
  props: {
    ...
    dbInfo: {
      type: Array,
      default: [{ enName: "", name: "暂无选项" }],
    },
    operation: {
      type: Symbol,
      default: Symbol.for("increAdd"),
    },
    formDisabled: {
      type: Boolean,
      default: false,
    ...
    return {
      ...
      basicForm: {
        ...
        applicationType: {
          ...
          option: {
            data: this.basicData["ApplicationType"],
          },
          help: [
            {
        ...
        cdrUrl: {
          ...
          used: false,
          rules: [
            { required: true, message: "Cdr地址不能为空", trigger: "blur" },
          ],
        },
        nlpUrl: {
          type: "input",
        ...
        tableBoundary: {
          ...
          rules: [
            { required: true, message: "增量范围不能为空", trigger: "blur" },
          ],
        },
        tableNames: {
          type: "cascader",
          ...
          help: [
            {
              title: "数据最后入库时间",
              content:
                "据入增量库时间， 数据在增量库如果有更新，指数据最后更新时间。来源于增量库表内SBMT_TIME 字段. ",
            },
            {
          ...
          help: [
            {
              title: "是否与历史数据合并",
              content:
                "是否依据uuid(患者主索引)将获得的新数据和科研应用内的数据合并",
            },
          ],
          rules: [
            {
          ...
          help: [
            {
              title: "实时性要求(分钟)",
              content:
                "若无新数据产生，任务会休眠1分钟后再次查询是否有新数据，依然无数据时就会休眠2分钟以此类推轮询。",
            },
          ],
          rules: [
            {
          ...
          help: [
            {
              title: "上次更新记录位置时间",
              content:
                "指定表中数据同步到数据入库的时间。修改这个时间可以指定再次启动任务起始数据位置。",
            },
          ],
        },
        cleanFlag: {
        ...
        cleanCdmv: {
          ...
          attributes: {
            "no-data-text":
              "无数据，①检查服务地址是否配置正确②在“清洗配置”处配置清洗规则。！",
            filterable: true,
          },
          events: {
        ...
        nlpFlag: {
          ...
          rules: [
            { required: true, message: "nlp结构化不能为空", trigger: "blur" },
          ],
        },
        encryptionFlag: {
          type: "select",
  ...
  methods: {
    ...
    findTable(rule, value, callback) {
      ...
      if (v.length && p.length) {
        ...
        callback(
          new Error(
            "任务无法运行， 请确保有 visits_visit_info表和patient_info表"
          )
        );
      }
    },
